...
namespace Octopus.Server.Extensibility.HostServices.Mapping
{
    public interface ICanonicalActionNameMapper
    {
        ...
        List<T> GetNamesFromIds<T>(string projectId, IEnumerable<T> source, Func<T, string> actionIds, Func<T, string, T> converter);
        ...
        List<T> GetIdsFromReferences<T>(string projectId, IEnumerable<T> source, Func<T, string> actionReferences, Func<T, string, T> converter);
    }
}